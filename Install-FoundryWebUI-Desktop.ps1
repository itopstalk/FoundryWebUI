...
<#
.SYNOPSIS
    Automated setup and update script for FoundryLocalWebUI on Windows 10/11.

.DESCRIPTION
    Installs all prerequisites and deploys FoundryLocalWebUI as an IIS website
    on Windows 10 or Windows 11 desktop editions.
    ...
    On first run:  Enables IIS, installs .NET, Foundry Local, creates IIS site, etc.
...
.EXAMPLE
    .\Install-FoundryWebUI-Desktop.ps1 -SkipPrerequisites
    # Fast update: just rebuild from source and redeploy.

.EXAMPLE
    .\Install-FoundryWebUI-Desktop.ps1 -Port 8080
#>
...
# Verify this is a desktop Windows (not Server)
if ($os.Caption -match "Server") {
    Write-Warning2 "This script is for Windows 10/11 desktop. For Windows Server, use Install-FoundryWebUI.ps1"
}
...
# ============================================================
# Step 2: Enable IIS with required features (DISM / Windows Optional Features)
# ============================================================
...
if ($restartNeeded) {
    Write-Warning2 "A restart is required to complete IIS installation."
    Write-Warning2 "Please restart your computer and re-run this script."
...
if ($w3svc -and $w3svc.Status -eq "Running") {
    Write-Success "IIS enabled and running"
} elseif ($w3svc) {
    Start-Service W3SVC
    Write-Success "IIS enabled and started"
} else {
    Write-Warning2 "W3SVC service not found. IIS may not have installed correctly."
    Write-Warning2 "Try enabling IIS manually: Settings > Apps > Optional Features > More Windows features > Internet Information Services"
}
...
# ============================================================
Write-Step "Step 4: Installing Microsoft Foundry Local"
...
if (Test-CommandExists "foundry") {
    ...
    try {
        ...
        $exited = $statusProc.WaitForExit(15000)
...
if ($isUpdate) {
    ...
    Write-Host ""
    Write-Host "  Your appsettings.json and system-prompts.json have been preserved." -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host "    3. .\Install-FoundryWebUI-Desktop.ps1" -ForegroundColor Yellow
    Write-Host ""
} else {
    Write-Host "========================================" -ForegroundColor Green
    ...
    Write-Host "    3. .\Install-FoundryWebUI-Desktop.ps1" -ForegroundColor Cyan
    Write-Host ""
}
